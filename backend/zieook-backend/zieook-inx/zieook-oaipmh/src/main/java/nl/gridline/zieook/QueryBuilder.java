...
public class QueryBuilder
{
	...
	private enum Verb
	{
		Identify, GetRecord, ListIdentifiers, ListMetadataFormats, ListRecords, ListSets
	}
	...
	private final String basesurl;
	private final String extendedBasesurl;
	...
	{
		basesurl = baseurl;
		extendedBasesurl = baseurl + QUESTION_MARK + VERB + EQUAL_SIGN;
	}
			...
			throws UnsupportedEncodingException
	{
		...
		if (set != null)
		{
			buffer.append(AMPERSAND).append(SET).append(EQUAL_SIGN).append(URLEncoder.encode(set, "UTF-8"));
		}
		return buffer.toString();
	}
	 ...
	 * @return the full query URL as a <code>String</code>
	 */
	public String buildListIdentifiersQuery(ResumptionTokenType token)
	{
		StringBuffer buffer = new StringBuffer(extendedBasesurl);
		buffer.append(Verb.ListIdentifiers);
		buffer.append(AMPERSAND).append(RESUMPTION_TOKEN).append(EQUAL_SIGN).append(token.getValue());
		return buffer.toString();
	}
	 ...
	 * @return the full query URL as a <code>String</code>
	 */
	public String buildListRecordsQuery(ResumptionTokenType token)
	{
		StringBuffer buffer = new StringBuffer(extendedBasesurl);
		buffer.append(Verb.ListRecords);
		buffer.append(AMPERSAND).append(RESUMPTION_TOKEN).append(EQUAL_SIGN).append(token.getValue());
	 ...
	 * @return the full query URL as a <code>String</code>
	 */
	public String buildListSetsQuery(ResumptionTokenType token)
	{
		StringBuffer buffer = new StringBuffer(extendedBasesurl);
		buffer.append(Verb.ListSets);
		buffer.append(AMPERSAND).append(RESUMPTION_TOKEN).append(EQUAL_SIGN).append(token.getValue());
