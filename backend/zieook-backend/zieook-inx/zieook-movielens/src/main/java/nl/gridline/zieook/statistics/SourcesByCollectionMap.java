 ...
 * SourcesByCollectionMap.java created 9 nov. 2011
 * <p />
 * Copyright, all rights reserved 2011 GridLine Amsterdam
 * @author <a href="mailto:job@gridline.nl">Job</a>
 * @version $Revision:$, $Date:$
 */
public class SourcesByCollectionMap extends TableMapper<CollectionSource, LongWritable>
{
	...
	private final CollectionSource keyOut = new CollectionSource();
	...
	{
		KeyValue collection = value.getColumnLatest(COLUMN_USERVIEW, ModelConstants.COLLECTION);
		KeyValue source = value.getColumnLatest(COLUMN_USERVIEW, ModelConstants.SOURCE);

		if (source != null && collection != null)
		{

			String cStr = Bytes.toString(collection.getValue());
			String sStr = Bytes.toString(source.getValue());

			keyOut.set(cStr, sStr);
			LOG.debug("map: <[" + cStr + "," + sStr + "]," + one + ">");
		...
		else
		{
			...
			if (source == null)
			{
				LOG.error("no key-value " + Bytes.toString(ModelConstants.SOURCE) + " for "
						+ Bytes.toStringBinary(value.getRow()));
			}
		}
	};
