 ...
 * AggregateAndRecommendReducer.java created 21 nov. 2011
 * <p />
			...
			Reducer<VarLongWritable, PrefAndSimilarityColumnWritable, VarLongWritable, RecommendedItemsWritable>
{
			...
			Context context) throws IOException, InterruptedException
	{
		...
		{
			predictionVector = predictionVector == null ? prefAndSimilarityColumn.getSimilarityColumn() : predictionVector
			...
			Context context) throws IOException, InterruptedException
	{
		...
		for (PrefAndSimilarityColumnWritable prefAndSimilarityColumn : values)
		{
			...
			numerators = numerators == null ? prefValue == BOOLEAN_PREF_VALUE ? simColumn.clone() : simColumn
					.times(prefValue) : numerators.plus(prefValue == BOOLEAN_PREF_VALUE ? simColumn : simColumn
					.times(prefValue));
